...
public class NXFile {
	...
	private static ByteBuffer getByteBuffer(String path) throws IOException {
		...
		ByteBuffer ret = fc.map(FileChannel.MapMode.READ_ONLY, 0, fc.size());
		raf.close();
		return ret;
	}
	 ...
	 * 
	 * @param path
	 *            The path the NX file is located at
	 ...
	 * 
	 * @param file
	 *            The RandomAccessFile representing the NX file
	 */
	...
	private void Parse() throws IOException, NXException {
		if (_ler.readUInt() != 0x32474B50)
			throw new NXException("Invalid NX file; magic not found");
		ParseStringTable();
		ParseBitmapTable();
		ParseMP3Table();
		...
		if (nodeCount < 1)
			throw new NXException("Invalid NX file; zero nodes!");
	...
	private void ParseStringTable() throws IOException, NXException {
		...
		if (strCount < 1)
			throw new NXException("Invalid NX file; zero strings in string table");
